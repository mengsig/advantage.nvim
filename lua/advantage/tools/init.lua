...
---named by `api_key_env`. Shared by the tool's run() and its enabled-gate.
local function web_search_key(cfg)
...
M._web_search_key = web_search_key
...
local function strip_html(s)
  s = (s or ""):gsub("<[^>]->", "")
  s = s:gsub("&quot;", '"'):gsub("&#0?39;", "'"):gsub("&amp;", "&"):gsub("&lt;", "<"):gsub("&gt;", ">")
    ...
    vim.system(cmd, { text = true, timeout = timeout_ms }, function(res)
      vim.schedule(function()
        ...
        if decoded.error then
          ...
          return cb("web_search error: " .. (type(e) == "table" and (e.message or vim.inspect(e)) or tostring(e)), true)
        end
